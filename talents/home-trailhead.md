...
- If the next move touches state, devices, rooms, or automations,
  activate `ha`.
- If the real task is delivering a message (no response needed),
  asking for a decision (async with callback, or sync blocking
  until reply), or resolving an incoming reply to an outstanding
