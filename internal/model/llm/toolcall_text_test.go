...
func TestParseTextToolCalls_FencedToolBlock(t *testing.T) {
	...
	content := "```tool\n{\"name\":\"activate_tag\",\"arguments\":{\"tag\":\"forge\"}}\n```"

	calls := ParseTextToolCalls(content, []string{"activate_tag", "deactivate_tag"}, profile)
	if len(calls) != 1 {
		t.Fatalf("len(calls) = %d, want 1", len(calls))
	}
	if calls[0].Function.Name != "activate_tag" {
		t.Fatalf("tool name = %q, want activate_tag", calls[0].Function.Name)
	}
...
func TestApplyTextToolCallFallback_PreservesUnknownToolShapeForRuntimeRepair(t *testing.T) {
	...
	ApplyTextToolCallFallback(resp, []string{"activate_tag", "deactivate_tag"}, profile)
...
func TestStripTrailingToolCallText_StripsTrailingFencedToolBlock(t *testing.T) {
	...
	cleaned := StripTrailingToolCallText(content, []string{"activate_tag"}, profile)
