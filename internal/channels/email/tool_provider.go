...
const flagsShapeDescription = "flags are the message's IMAP flags as the server spells them: \\Seen (read), \\Answered (replied to), \\Flagged (flagged), \\Draft, \\Deleted, and keywords. A flag colour is \\Flagged plus the keywords $MailFlagBit0-2, which a client that colours flags shows as a colour and every other client as a plain flag. " +
	...
	"flag_label names the label whose flag the message carries, and appears only when Thane's own record shows Thane wrote that flag for the label on this very message in this folder and it still carries it as written: that flag is the label's mark, not a request for the operator's attention. A flagged message without flag_label is flagged for attention, by the operator or a pass, whatever its colour; that includes a second copy of a message Thane marked, and a message the operator moved after Thane marked it, because Thane's claim does not follow either. "
...
func uidsParameters() (map[string]any, map[string]any) {
	return map[string]any{
		...
		"maxItems":    maxBatchUIDs,
		"description": "Message UIDs (integers) from an email_list or email_search result in the same account and folder, at most 100 per call; split a larger set across several calls. Required unless uid is provided.",
	}, map[string]any{
		"type":        "integer",
	...
	return []*tools.Tool{
		...
		},
		{
			...
			Description: "Reply to a message by UID, preserving In-Reply-To and References so the reply threads in the recipient's client. " +
				...
				"The one exception drafts rather than refuses: on an account whose Email Accounts entry shows draft_gate: relaxed, a reply to list mail (a List-Id or a Precedence of bulk or list), not auto_submitted, whose own to or cc names this account's address is drafted, with decision.route personally_addressed_list_reply; list mail that reached this account only through a list address, every auto_submitted message (automatic replies, bounces, notifications), a message marked Precedence junk, which classic autoresponders set, and a message whose only list marks are fields such as List-Unsubscribe, which a sender adds to its own mailings, stay refused, and the refusal says which. " +
		...
		},
		{
			...
			Description: "Move messages to another folder in the same account. Provide uids (array of integers) or uid (single integer), folder (the source, default INBOX), and exactly one target: " +
				...
				"In a turn the operator is not present for (attended: false in the Email Accounts block), each account allows moves only into the folders its mailbox.move_into names, shown as move_into in its Email Accounts entry whenever it is limited, and back to INBOX out of one of those; any other target is refused and nothing moves. There, an account whose entry shows owner: operator allows only its junk folder unless the operator configured more. " +
				...
				"Returns JSON {action: moved|refused, account, source_folder, destination_folder, uids, destination_uids, destination_uids_known, uids_not_found, moved:[{uid, destination_uid, message_id, from, trust_zone}], refused:[{uid, from, trust_zone, reason, recovery}], moved_omitted, refused_omitted, note}; action is refused only when every message was refused and nothing moved. " +
				"A call takes at most 100 UIDs, and the result stays within 16 KB: account, source_folder, destination_folder, note, from, message_id, and reason over 256 bytes are cut and end with …[cut], and when the entries would pass the limit, the last moved_omitted entries of moved, then the last refused_omitted of refused, carry only their UIDs. uids, destination_uids, and every entry's uid and destination_uid are complete unless the server reports moving more messages than the call sent; then the lists are cut short from the end and note says so. " +
				...
				"The Email Accounts block's recent_operations records each move with both UID lists, at most 10 of each with the rest counted, or with destination_uids unknown when the server confirmed nothing; list destination_folder for any it does not show. To undo a move, move destination_uids from destination_folder back to the original source_folder, with destination_role inbox when that was INBOX; in a turn the operator is not present for, a limited move_into refuses a move back into any other folder it does not list, so then tell the operator where the messages are. " +
				"In every turn, a destination the account lacks is refused with the account's real folder list, the account's drafts folder is never a destination or a source (when listing the account's folders fails while finding it, the move is refused and nothing moves), and an account whose access is read refuses this tool.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"uids": uidsParam,
