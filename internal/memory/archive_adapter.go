...
// Deprecated: Use SessionMetadataGenerator for rich metadata.
type SessionSummarizer func(ctx context.Context, messages []ArchivedMessage) (string, error)

// SessionMetadataGenerator produces rich, structured metadata for a session.
// Returns title, tags, and full metadata in one LLM call.
type SessionMetadataGenerator func(ctx context.Context, messages []ArchivedMessage, toolCalls []ArchivedToolCall) (*SessionMetadata, string, []string, error)
...
type ArchiveAdapter struct {
	...
	summarizer  SessionSummarizer        // optional — generates session summaries on close (legacy)
	metadataGen SessionMetadataGenerator // optional — generates rich metadata on close
...
// Deprecated: Use SetMetadataGenerator for rich metadata.
func (a *ArchiveAdapter) SetSummarizer(fn SessionSummarizer) {
	a.summarizer = fn
}

// SetMetadataGenerator configures a function to generate rich session metadata on close.
// If set, this takes priority over the legacy summarizer.
func (a *ArchiveAdapter) SetMetadataGenerator(fn SessionMetadataGenerator) {
	a.metadataGen = fn
}
...
func (a *ArchiveAdapter) EndSession(sessionID string, reason string) error {
	...
	// Generate metadata asynchronously — don't block the caller
	if a.metadataGen != nil {
		go a.generateSessionMetadata(sessionID)
	} else if a.summarizer != nil {
...
// generateSessionMetadata creates rich AI-generated metadata for the session.
func (a *ArchiveAdapter) generateSessionMetadata(sessionID string) {
	...
	toolCalls, err := a.store.GetSessionToolCalls(sessionID)
	if err != nil {
		toolCalls = nil // not fatal — proceed without tool call data
	}

	meta, title, tags, err := a.metadataGen(ctx, messages, toolCalls)
	if err != nil {
		a.logger.Warn("failed to generate session metadata",
			"session", ShortID(sessionID),
			"error", err,
		)
		return
	...
	if err := a.store.SetSessionMetadata(sessionID, meta, title, tags); err != nil {
		a.logger.Warn("failed to save session metadata",
			"session", ShortID(sessionID),
			"error", err,
		)
		return
	...
	a.logger.Info("session metadata generated",
		"session", ShortID(sessionID),
		"title", title,
		"tags", len(tags),
	)
