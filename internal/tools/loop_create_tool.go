...
func (r *Registry) registerThaneLoopCreate() {
	r.Register(&Tool{
		...
		Description: "Create and launch a durable, reusable loop. This is the always-on front door for standing up recurring work; for the full lifecycle afterwards (inspect, edit, relaunch, reparent) activate the `loops` capability. " +
			...
			"bindings restricts which named shared resources the loop may reach (currently forge_account); a caller already inside a binding carries it into the new loop and cannot override it or place the loop under an incompatible container. " +
	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"bindings": map[string]any{
				...
				"description":          "Optional resource instances this loop is restricted to. Recognized key: forge_account, naming the forge account every forge tool resolves to while refusing all others. Bindings inherit from containers with outermost-ancestor-wins semantics. A caller already inside a binding cannot omit or override it, and cannot create the loop under a container that resolves that binding differently.",
			},
