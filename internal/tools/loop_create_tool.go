			...
			"output": map[string]any{
				...
				"properties": map[string]any{
					...
					"facets": map[string]any{
						"type":        "array",
						"items":       map[string]any{"type": "string", "enum": []string{"signal", "teaser", "digest"}},
						"description": "Publish projections alongside the full body, so each consumer takes the shape it needs. signal is the compact outward-facing reason to spend attention, teaser gives a roomier reason to look, and digest carries enough context to act. signal anchors every faceted output; add teaser and digest only when consumers need them. Declaring any swaps the loop's generated tool from replace_output_* to publish_output_*, which takes one argument per declared projection plus full.",
					},
