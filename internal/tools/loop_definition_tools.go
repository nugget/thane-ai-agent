...
func (r *Registry) registerLoopDefinitionTools() {
	...
	r.Register(&Tool{
		Name:        "loop_definition_patch",
		Description: "Apply a small, field-scoped edit to one existing dynamic loop definition in the persistent loops overlay, without resending the whole spec. Use this for incremental changes — enable supervisor, set supervisor_prob, rewrite the profile instructions, retune the sleep envelope, pin a model — instead of recomposing the entire definition with loop_definition_set. Only the keys you include under patch change; every other field is preserved exactly. Config-owned definitions are immutable. Structural and list-valued fields (operation, completion, outputs, subscriptions, tags) are not patchable — use loop_definition_set to rewrite those; to move a loop use loop_reparent, and to change run state (active/paused) use loop_definition_set_policy. The edit is persisted immediately, but a running service loop keeps its launched-time config until a full relaunch (process restart, or stop_loop then loop_definition_launch) — retunes apply then, not on the loop's next wake.",
		// Same literal-payload hazard as loop_definition_set/lint: the merged
		// spec carries outputs[].ref strings, and the patch values are stored
		...
		Parameters: map[string]any{
			...
			"properties": map[string]any{
				...
				"patch": map[string]any{
					"type":        "object",
					"description": "Field-scoped changes. Only the keys you include are changed; all other fields of the definition are preserved exactly. Include at least one.",
					"properties": map[string]any{
						"task": map[string]any{
							"type":        "string",
		...
		Handler: r.handleLoopDefinitionPatch,
	})
