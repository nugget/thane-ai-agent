...
func (r *Registry) registerLoopDefinitionTools() {
	...
	r.Register(&Tool{
		...
		Description: "Create or replace one dynamic loop definition in the persistent loops overlay. This cannot modify config-owned definitions. The saved definition view includes warnings for common service-loop authoring mistakes. The spec uses human-facing strings for durations and retrigger mode. The write is persisted immediately, but a running loop keeps its launched-time config until a full relaunch (process restart, or stop_loop then loop_definition_launch) — a replaced spec applies then, not on the loop's next wake.",
	...
	r.Register(&Tool{
		...
		Description: "Change a subset of fields on one existing dynamic loop definition in the persistent loops overlay, without resending the whole spec. Use this for incremental edits — enable supervisor, set supervisor_prob, rewrite the profile instructions, retune the sleep envelope, pin a model — instead of recomposing the entire definition with loop_definition_set. Pass name plus any subset of the editable fields below; every field you omit is preserved exactly. Config-owned definitions are immutable. Structural and list-valued fields (operation, completion, outputs, subscriptions, tags) are not editable here — use loop_definition_set to rewrite those; to move a loop use loop_reparent, and to change run state (active/paused) use loop_definition_set_policy. The edit is persisted immediately, but a running loop keeps its launched-time config until a full relaunch (process restart, or stop_loop then loop_definition_launch) — retunes apply then, not on the loop's next wake.",
