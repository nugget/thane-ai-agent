...
func (r *Registry) registerMQTTSubscriptionTools() {
	...
	r.Register(&Tool{
		Name: "mqtt_wake_add",
		Description: `Add a runtime MQTT wake subscription. When a message arrives on the given topic, the agent wakes with the specified routing configuration. The subscription is immediately active on the broker and persists across restarts.
