	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"audience": map[string]any{
				...
				"description": "How far this output's content reaches inside Thane. agent (default) allows projection into search results, context injection, and ambient surfaces; subscribers reaches only loops holding a subscription to it, and is never advertised and excluded from search by default (doc_search returns it on include_restricted, or when filtering the audience key explicitly); private keeps it to this loop's own context and explicit reads by ref. working_notes outputs are private automatically. Every value stays inside Thane and none of them permit anything to leave for an external surface — that is a separate declaration. This is context hygiene, not secrecy: operators and the archive still see the document.",
			},
