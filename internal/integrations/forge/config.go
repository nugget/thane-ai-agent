...
type Config struct {
	...
	// SubscriptionCheckInterval is how often (in seconds) to poll
	// followed repositories for releases and commits. Zero disables
	// polling while keeping the management tools available.
	SubscriptionCheckInterval int `yaml:"subscription_check_interval"`

	// MaxSubscriptions limits runtime-managed repository subscriptions.
...
func (c Config) Validate() error {
	if c.SubscriptionCheckInterval < 0 {
		return fmt.Errorf("forge subscription_check_interval must be non-negative")
	}
	if c.MaxSubscriptions < 0 {
		return fmt.Errorf("forge max_subscriptions must be non-negative")
	}
