...
type providerEntry struct {
	build    func(settings map[string]any) (certmagic.DNSProvider, error)
	defaults dnsDefaults
}

// providers is the registry of DNS providers the front door can solve
// DNS-01 challenges through. libdns providers are compiled in, so this
// is the one seam of the certmagic pass-through that is not generic: a
// name here selects a Go module. Add providers as operators need them.
...
func newProvider(name string, settings map[string]any) (certmagic.DNSProvider, dnsDefaults, error) {
