...
// LISTEN_FDS saying how many, LISTEN_PID naming the intended child so a
// stale environment is never mistaken for a handoff, and LISTEN_FDNAMES
// naming each one. Ports 80 and 443 need privilege Thane must never
...
func adoptListeners(env func(string) string, pid int, logger *slog.Logger) (map[string]net.Listener, error) {
	...
	if rawPID := strings.TrimSpace(env("LISTEN_PID")); rawPID != "" {
		if intended, err := strconv.Atoi(rawPID); err != nil || intended != pid {
			logger.Warn("ignoring inherited listeners addressed to another process",
