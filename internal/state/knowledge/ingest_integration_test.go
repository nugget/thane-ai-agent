...
func (m *mockIngestEmbedder) Generate(ctx context.Context, text string) ([]float32, error) {
	m.calls++
...
`
	...
	ingester := NewMarkdownIngester(store, mock, "test:integration", CategoryArchitecture)
