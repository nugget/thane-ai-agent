...
func TestArchivistUsesCanonicalContactDossiers(t *testing.T) {
	...
	for _, want := range []string{
		...
		"`contact_dossier_write` refuses it and names the full citation if one session matches",
		"In a `dossiers:` document nothing checks the form, so do the same lookup yourself",
		"`archive_session_transcript` takes the prefix",
		"search `archive_search` for the claim's own words",
		"Evidence you still cannot pin to one session goes under Open Questions",
		"Never reword it as prose about a prefix",
