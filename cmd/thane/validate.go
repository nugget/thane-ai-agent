...
func writeTLSText(w io.Writer, cfg *config.Config, err error) {
	...
	fmt.Fprintf(w, "  binds :%d (public %d); listeners a supervisor hands down (LISTEN_FDS) override the bind at serve time\n",
		cfg.TLS.HTTPS.Port, cfg.TLS.HTTPS.EffectivePublicPort())
}
