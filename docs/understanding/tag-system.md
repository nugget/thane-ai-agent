...
through `activate_tag` / `deactivate_tag` — and a
...
| **Core tool** | A boolean flag on a `Tool`. | [`tools.Tool.Core`][core-tool]. | Tool-level, not tag-level. Survives all tag filters. Used for meta-tools (`activate_tag` itself, etc.) and for `RuntimeTools`. |
...
`inspect_tag` returns the per-tool breakdown of a single tag.
...
| `activate_tag` / `deactivate_tag` semantics | [`internal/tools/capability_tools.go`][cap-tools] and the scope code |
