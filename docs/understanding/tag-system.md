...
| **Virtual model** | A user-facing `thane:*` model name (`thane:premium`, `thane:ops`) selected via the client's model field. | [`router/virtual_models.go`][virtual-models], [Routing Profiles](../operating/routing-profiles.md). | Resolves to a `LoopProfile`. Express user/integration execution intent. Prefer this term over the older "routing profile" in new prose. |
