...
| **Core tag** | Tags pinned in every scope by config. | [`Executor.SetCoreTags`][exec-core], config `capability_tags.*.core` (legacy alias `always_active` accepted with deprecation warning). | Tag-level, not tool-level. Re-seeded each run. |
