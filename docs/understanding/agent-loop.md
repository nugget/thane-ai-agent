...
- **Faceted maintained document** outputs — a maintained document that
  ...
  projections it was published from. `signal` anchors the contract as the
  compact outward-facing reason to spend attention; `teaser` is an optional
  roomier hook, while `digest` carries enough context to act.
