...
- [ ] **GitHub release notes authored at `docs/releases/v<version>.md`** — the committed repository file is the publication source (`release-github` reads it, strips the document-only H1, and refuses missing or malformed notes; see Build readiness and the archival contract in [docs/releases/README.md](releases/README.md)). Never draft on the release object alone: a GitHub-only body has no signed history and the archive contract post-dates that habit.
