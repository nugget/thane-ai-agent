...
inclusion. 106 findings survived verification. Live Anthropic
...
6. **Loader validation PR** — refuse unknown `kind:` values; warn on
   inert `teaser:`; next_tags/body coverage assertion.
7. Remaining per-file economy trims batched by cluster (comms, ops,
   home/docs, small leaves) using the appendix below.

## Verification status

Findings from the trailheads, foundation, loops, and home_docs passes
were adversarially verified by an independent pass (quote re-checked,
tool names resolved against the catalog, behavior claims re-read in
code). The comms, ops, small_leaves, caching, and assembly passes lost
their verifiers mid-run; every **high-priority** finding from those
groups was then independently re-verified by hand against the repo and
the live Anthropic documentation before inclusion here (the caching
code claims at anthropic.go:754-1004 and core_context.go:256-278 in
particular). Medium/low findings from those five groups are
single-pass auditor output — treat the appendix entries as strong
leads, and re-verify the cited lines before acting on any of them.
...
finding is a size finding.

### Trailheads (all ten, cross-file)

- **[high/border]** `talents/knowledge-trailhead.md` — first branch bullet, lines 15-16
  - The trailhead routes dossiers and semantic paths to `files`, directly contradicting the files/documents border doctrine both leaves feature prominently near their tops.
  - Fix: Rewrite the first two bullets to match the leaf border: "If the truth has a semantic ref (`kb:...`, `dossiers:...`), activate `documents`." / "If it lives at a raw workspace path, activate `files`." The current wording sends a model holding a kb: ref into `files`, whose own leaf immediately bounces it back — one wasted tag activation per mis-route. Likely architecture-stale from before the PR-D documents split (docs/talent-authoring.md notes documents.md was restructured in PR-D); the file's own closing line ("Preserve references like `kb:article.md` exactly") sits beside the wrong door.

- **[high/economy]** `talents/awareness-trailhead.md` — feed bullet 2 (Wake, lines 37-47) and the mechanics bullets (lines 62-81) vs loops.md 'Choose stream wiring by attention cost' (lines 141-170) *(~180 tok/activation)*
  - Three blocks duplicate loops.md's 'Choose stream wiring by attention cost' section nearly verbatim; both files co-inject whenever `loops` and `awareness` are active together — the standard core-loop authoring flow, since loops-trailhead's next_tags point at awareness — though not inside a running watcher loop itself (the canonical curate-watcher tag set in loops-examples.md line 154 is [home, ha, awareness, documents], without `loops`).
  - Fix: Pick one canonical home per mechanic. Subscription mechanics belong on the awareness side; cut loops.md's stream-wiring bullet to a short cross-reference with a trigger ("entity subscriptions are the awareness surface; `wake: true` when the loop should act on change — mechanics live under `awareness`"), or if awareness is slimmed per trailheads-4, invert the direction. Either way, stop paying ~180 tokens twice whenever both tags are loaded — which is every loop-authoring session.
...
- **[high/economy]** `talents/loops-trailhead.md` — the thane_loop_create paragraph, lines 41-50 *(~100 tok/activation)*
  ...
  - Fix: Trim the trailhead paragraph to the fork-relevant core: "`thane_loop_create` is the shorter front door to the durable path — one output document, facets, a working-notes document, `dry_run: true` to preview. Scaffolding, seeding, and the `loop_definition_set` escape hatch are doctrine below." The doctrine keeps the full mechanics. ~100 tokens saved on every loops activation.

- **[medium/anti-pattern]** `talents/awareness-trailhead.md` — whole file (5281 bytes ≈ 1320 tokens; the nine peer trailheads run ~180-970 tokens, median ~410)
  - The file is leaf doctrine wearing kind: trailhead — a full parameter reference for the subscription surface with no 'choose the next move' fork; of next_tags [ha, home, notifications], only notifications gets a body trigger (line 91-92).
  - Fix: Split: a ~300-token awareness trailhead keeping the three-question frame plus the bounces (one-shot check → visible HA tool; delivery → `notifications`; device work → `ha`), and an awareness-doctrine.md node (awareness.md is taken by the persona-tagged foundation file, and `<tag>-doctrine.md` is the documented convention when a filename is contested) holding the parameter mechanics. Co-load makes the split token-neutral, but it restores the trailhead grammar, gives the mechanics a home that trailheads-2's dedup can point at, and stops the model reading a reference manual when it only needed the fork.

- **[medium/assembly]** `talents/operations-trailhead.md` — frontmatter next_tags (line 5) vs body bullets (lines 14-46)
  - next_tags and body routing diverge in both directions across five group files, and the menu renders next_tags as bare names (render.go line 59: " next: %s"), so the trailhead itself supplies no disqualification test for an entry with no body trigger — only the target tag's own one-line catalog description elsewhere in the menu.
  - Fix: Align both directions in each file: every body-routed tag joins next_tags (operations should add session, ha, awareness, companion — all real tags in catalog_tags.go), and every next_tags entry gets a one-line disqualification trigger in the body (a real trigger per the corpus grammar, not the bare restatement talent-authoring.md flags) or is dropped. For interactive/people, note message_channel is Protected in catalog_tags.go ("not manually activatable"), so dropping it is the likely right alignment; message-channel: same choice for interactive; awareness: same for ha/home. TestRepoTrailheadNextTagsResolve only checks resolution, not coverage, so this drift is invisible to CI.
...
- **[medium/economy]** `talents/loops-trailhead.md` — ## Where to go next, lines 52-63 *(~70 tok/activation)*
  ...
  - Fix: Cut bullet 1 entirely (the doctrine needs no advertisement to a reader who already has it). Convert bullets 3-4 to trigger form or drop them: "Activate `loops_examples` before composing any spec — adapt the closest recipe, don't write from the schema." is the only line that earns the section.

- **[medium/economy]** `talents/development-trailhead.md` — ## Workspace gotchas, lines 34-41 *(~70 tok/activation)*
  - The gotcha section duplicates a border row files.md already features in its top disambiguation table, and short-circuits the file's own 'shell is the escape hatch, activate it first' routing by prescribing a bare exec call (exec is tagged [shell] in catalog.go, so it is not reachable without the activation the bullet above requires).
...
- **[medium/economy]** `talents/operations-trailhead.md` — first branch bullet, lines 16-21 *(~50 tok/activation)*
  - The request_core_attention bullet leads the trailhead with six lines of mechanism already carried by loops.md and notifications.md, ahead of the routing forks most readers arrive for.
...
- **[low/economy]** `talents/message-channel-trailhead.md` — ## The conversation can move while you work, lines 43-75 *(~60 tok/activation)*
  - The mid-flight-arrival section (unique, load-bearing content) restates its own framing: the not-a-runtime-instruction and read-it-as-their-next-words points from the framing paragraph (lines 53-55) are each made again in the closing paragraph (lines 68-71).
  - Fix: Keep the marker quote, the three bold bullets, and the batch paragraph; cut the "What arrival never means" paragraph down to its one novel clause ("A tangent is not a stop order" is already in bullet 3, line 63) — e.g. end the bullet list with: "Arrival never means restart the turn; weigh it the way you would weigh their last sentence." ~60 tokens per activation of one of the highest-frequency tags in the system.
...
- **[low/anti-pattern]** `talents/people-trailhead.md` — contacts bullet, lines 18-19 *(~20 tok/activation)*
  ...
  - Fix: Delete the sentence. It matches the co-mingling signal in talent-authoring.md ("this depends on the deployment", line 457) exactly: the model sees the actual menu at runtime, so a remapped name explains itself; a host that remaps it can carry the note in its overlay.
...
- **[low/economy]** `talents/interactive-trailhead.md` — session bullet, lines 18-22 (paired with operations-trailhead.md lines 35-38) *(~40 tok/activation)*
  - Both interactive and operations trailheads enumerate all four session operations with parentheticals, duplicating the session leaf's own catalog (session.md's destructive/non-destructive table names all four tools) in two places when only the trigger and the safety constant earn trailhead space.
  - Fix: Keep the safety qualifier (the audit doc's email-cycle learning, tool-and-talent-audit.md lines 319-326, explicitly endorses compressing the safety constant — not the op catalog — into the trailhead bullet) and drop the four-op enumeration in both files: "If the turn is about the session's own lifecycle — resetting only on explicit user request — activate `session`." The leaf owns the operation menu.

- **[low/tone]** `talents/message-channel-trailhead.md` — bullet list under 'Choose the next move deliberately:', lines 14-41
  - The list mixes bare posture imperatives with activation forks under a chooser header — in every other trailhead that carries this chooser list, each bullet is a conditional fork ('If X... activate/use Y'); message-channel alone interleaves unconditioned stance bullets.
...
- **[high/stale]** `talents/presence.md` — ## Anticipations bridge the gap (lines 21-27) *(~75 tok/activation)*
  ...
  - Fix: Rewrite the section mechanism-neutral around what replaced it (task_schedule's required `action` field — confirmed in internal/tools/tools.go lines 926-935 — and loop working notes): before -> 7 lines naming a dead lifecycle; after -> ~2 sentences, e.g. "A future wake without its why is an arbitrary alarm. When you schedule one, put the reason in the wake itself — 'Dan's flight lands at 14:45, check status, offer pickup' — so you arrive knowing what matters." Saves ~75 tokens per persona turn (section is ~500 bytes ≈ 125 tokens; the replacement ~50) and stops teaching a scrapped architecture.

- **[high/anti-pattern]** `talents/working-memory.md` — whole file; tool reference at line 7 *(~460 tok/activation)*
  ...
  - Fix: Retag to `tags: [memory]` (moving ~460 tokens — the file body is ~1846 bytes — out of the 1h-cached persona prefix into the 5m-cached tagged block) and update memory.md's two pointers (lines 220-223 and 236-240) that call it "the working-memory.md foundation talent ... loads wherever the persona is present"; talents/README.md ("Six foundation files today", line 70) and docs/talent-authoring.md (lines 23-24) list it as foundation prose and must be updated too. Note the counter-signal: memory.md documents the craft/operations split as deliberate, so this is a design decision to revisit, not a mechanical fix — the compromise is keeping only a 2-3 sentence identity-level kernel (capture texture before compaction) in persona and moving the what/when lists to the memory tag.
...
- **[medium/assembly]** `talents/foundation.md` — whole file vs loader ordering (internal/model/talents/loader.go lines 120, 358-367, 413-418)
  - foundation.md is the corpus preamble telling the model how to read talents, but alphabetical file ordering renders it deep in the Behavioral Guidance block — after awareness, communication, and delegation have already been read.
  - Fix: Make the framer lead: special-case foundation first among turn-shape talents in talentOrderKey (or add an order/weight frontmatter key). Zero steady-state token cost (one-time cache invalidation of the stable prefix aside); the fix is loader-side, plus the README claim becomes true instead of drifted. Deterministic ordering is preserved, satisfying docs/model-facing-context.md's "sort deterministically" rule.

- **[medium/economy]** `talents/communication.md` — ## What to trust (lines 23-30) and lines 17-19 *(~60 tok/activation)*
  - Energy-mirroring, anti-summarize, and compress-commands doctrine is duplicated between always-on communication.md and the interactive-tagged interactive-communication.md.
...
- **[medium/economy]** `talents/working-memory.md` — ## When texture is actually a fact (lines 37-46) *(~70 tok/activation)*
  - The closing section restates memory.md's signature noticing doctrine nearly verbatim rather than being the short border pointer the border-doctrine convention calls for.
  - Fix: Both-sides border doctrine is intentional, but the persona side only needs the redirect, not the doctrine: before -> ~9-line section; after -> "Texture that is actually a stable fact (a preference, a layout, a routine) belongs in `remember_fact` under `memory` — working memory dies with the session." Saves ~70 always-on tokens. (Dissolves entirely if foundation-2 is adopted.)

- **[medium/economy]** `talents/foundation.md` — lines 10-12 and 22-25 *(~70 tok/activation)*
  - Paragraphs two and five restate what paragraphs one and three already establish, in a file that rides every persona turn.
  - Fix: Merge: before -> 7 paragraphs; after -> cut lines 10-12 entirely and compress lines 22-25 to one sentence ("When a line sounds like an editor describing a document, prefer the self-facing reading — 'you' is now you."). Keeps the closer intact; saves ~70 tokens of the file's ~290 (1181 bytes).
...
- **[low/economy]** `talents/awareness.md` — ## Time, line 21 *(~30 tok/activation)*
  ...
  - Fix: Before -> the 41-word hedge chain; after -> "Never persist a delta — it rots the moment it's written. Durable prose gets an absolute time, and only when the time itself is the point." The per-surface deferral adds nothing decidable here (loops.md lines 76-88 already own the temporal-template mechanism for durable curated prose). ~25-30 tokens.
...
- **[low/economy]** `talents/awareness.md` — ## Curiosity, lines 39-41 *(~20 tok/activation)*
  ...
  - Fix: Drop the line-41 restatement (end the paragraph at "the default is curiosity"); the philosophical framing survives once. ~20 tokens (the dropped sentence is ~74 bytes) from the group's largest file (awareness.md, 3414 bytes, vs communication.md's 2841).
...
- **[low/border]** `talents/awareness.md` — filename vs the `awareness` capability tag
  ...
  - Fix: No content change needed now (the loader keys off frontmatter, and content overlap is nil); note it so any future awareness-tag doctrine gets `awareness-doctrine.md` (the README's sanctioned disambiguation, cf. interactive-doctrine.md alongside interactive-communication.md), or rename the persona file (e.g. perception.md) in a housekeeping pass.


### Loops corpus

- **[high/doc-code-drift]** `talents/loops-examples.md` — loops_examples_curate node, "## Tags scope the loop's tools", lines 101-106 (sentence at 104-105)
  - The curate node's "Omit to inherit the core tag set" contradicts both loops-tagging.md and the runtime, which skips tag filtering entirely on an empty tag set — handing the loop the full base registry, not core-only.
  - Fix: Replace the sentence with loops-tagging's position: "Omitting `tags` skips scoping entirely — name an explicit narrow set for every service loop." Both talents co-load on the walked trail (loops -> loops_examples -> loops_examples_curate), so the model currently sees the same fact in two conflicting shapes. Also flag the thane_loop_create schema description ("Omit to use only core tags.", internal/tools/loop_create_tool.go:958) for the tool-group auditor — it matches the wrong side of this contradiction.
...
- **[high/anti-pattern]** `talents/loops-examples.md` — loops_examples_curate_dashboard node, JSON block lines 140-170
  ...
  - Fix: Make the primary literal a thane_loop_create call (`entities`, singular `output` with `facets` and `initial`); keep the hand spec as a clearly-labeled secondary block or drop it and point at loops_examples_advanced. A model minimally adapting today's literal into the front-door call produces invalid field names.

- **[high/economy]** `talents/loops-examples.md` — curate node lines 74-78 and dashboard node lines 117-123 (vs loops.md lines 43-48) *(~150 tok/activation)*
  - The facets rationale appears three times on the trail the model actually walks: near-verbatim twice (loops.md doctrine and the dashboard leaf, both carrying the status_line/teaser/digest ladder plus "blind truncation") and in compressed form a third time in the curate sub-trailhead's question 1.
...
- **[high/economy]** `talents/loops-examples.md` — dashboard "## Publishing" section, lines 185-209 (vs loops.md lines 58-69) *(~150 tok/activation)*
...
- **[medium/economy]** `talents/loops-examples.md` — circle node, wake-routing paragraphs lines 287-307 (vs loops.md lines 162-175) *(~120 tok/activation)*
  ...
  - Fix: The circle node keeps one routing line per door plus its unique payload (the worked mqtt example): "Single-entity change -> `wake: true` on the entry. Producer event -> `wake_loop` on the producer tool. HA-side derived condition -> `mqtt_wake_add` + HA automation (worked example below)." Delete the restated rationale sentences.
...
- **[medium/economy]** `talents/loops-examples.md` — circle node, "Why this shape is the canonical event-bridge", lines 472-488 *(~150 tok/activation)*
  ...
  - Fix: Cut the three rationale bullets; keep one sentence of the generalization: "The same two-artifact pairing fits any 'HA notices -> Thane decides' workflow — sump pump cycling, garage door open late, freezer drift." The division-of-labor insight is already implicit in the worked example's structure; docs/talent-authoring.md's "Developer-doc co-mingling" anti-pattern covers exactly this kind of why-it-exists prose.

- **[medium/economy]** `talents/loops-examples.md` — dashboard "Where the reasoning goes", lines 216-227 (vs loops.md "Where the thinking goes", lines 121-139) *(~100 tok/activation)*
...
- **[low/economy]** `talents/loops.md` — lines 185-188 (also loops-examples.md circle step 3 lines 253-257 and the force_supervisor aside on wake_loop, lines 400-402) *(~50 tok/activation)*
  - The supervisor-turn cost warning appears three times across the group — twice for request_core_attention (loops.md and circle step 3, near-verbatim) and once as the `force_supervisor` variant on wake_loop — and a fourth time near-verbatim in the tool's own description, which every caller sees at call time since request_core_attention is Core.
...
- **[low/other]** `talents/loops.md` — frontmatter, line 2
  - `kind: doctrine` is an undeclared frontmatter value — the README's contract says kind is "trailhead or empty" and kind.go declares only trailhead (+ deprecated entry_point); the loader passes the unknown value through unvalidated (CanonicalKind returns the trimmed raw, and loader.go:362 keys only on KindTrailhead), so it works by accident, not by contract.
...
- **[high/assembly]** `internal/model/talents/loader.go` — talentsFromBlocks (lines 291-332), shouldIncludeTalent (lines 437-447)
  - Multi-node talent files load per node, not per file: ha.md is three independent activations (ha ~3,150 tokens, ha_control ~1,100, ha_automate ~1,680 from frontmatter-stripped content of 12,612/4,391/6,707 bytes) and documents.md is eight (1,003-3,918 bytes, ~250-980 tokens each), so neither file is ever a single 24KB/20KB injection.
  - Fix: No change needed — but note the corollary for auditing: no tool in catalog.go carries tag ha_control or ha_automate (every HA-family tool carries Tags:["ha"] — e.g. line 154 ha_call_service, line 236 ha_automation_create — except ha_notify, tagged notifications), so activating ha_control/ha_automate loads guidance only, never additional tools; the control tools are already callable the moment `ha` is active. The root's framing ("one deliberate step further in") gates doctrine, not capability. Also note tags accumulate (tag_activate only adds; nothing auto-drops ha), so in practice ha_control/ha_automate co-load with the ha root — duplication across those borders is a real per-turn cost.

- **[high/doc-code-drift]** `talents/ha.md` — ha_automate node, "## Author a new automation" (lines 515-592)
  - The automation-authoring section — the leaf's centerpiece — never names its tool: `ha_automation_create` exists in the catalog (catalog.go:236) and is promised by the node's teaser ("create", line 461) but appears nowhere in ha.md, while every sibling section names its tool in the heading or first line.
  - Fix: Name the tool where every sibling does: retitle to "## Author a new automation — `ha_automation_create`" or open with "`ha_automation_create` takes a `config` + `metadata` pair:". The leaf success criterion (docs/talent-authoring.md: adapt the JSON into a real tool call) fails without the tool name — the model must fish the schema list for the verb the talent taught it to want. The name passes TestRepoTalentToolReferences since it is a real catalog key.
...
- **[medium/border]** `talents/ha.md` — ha root, "## Presence and zones" (lines 306-320) vs "## Cross-references" presence bullet (lines 327-329)
...
- **[medium/economy]** `talents/ha.md` — ha root, intro paragraph (lines 18-22) and "## When you need to act, step further in" (lines 279-288) *(~90 tok/activation)*
  - The two-branch fork is stated twice in the root, and the second statement near-verbatim restates the branches' teaser frontmatter — the restating-next_tags anti-pattern named in docs/talent-authoring.md — paid on every `ha` activation.
  - Fix: Delete the section but keep the one-line fork marker where the read tools end: "Everything above reads; to act, activate `ha_control` or `ha_automate` — each costs more when wrong." That line is load-bearing, not optional: verification shows the always-rendered tag menu surfaces only TagKindMenu trailheads and protected tags (selectCapabilityMenuEntries, toolcatalog/render.go:16-28), and ha_control/ha_automate are talent-declared leaves whose teasers render nowhere else automatically — so the intro paragraph plus one marker line must carry the fork, and they already do; the deleted section's extra detail lives in the leaves themselves.

- **[medium/economy]** `talents/ha.md` — ha root: ha_device (lines 91-98), ha_get_state include paragraph (lines 161-165), "**Hidden entities.**" block (lines 194-203) *(~80 tok/activation)*
  ...
  - Fix: Keep the dedicated "**Hidden entities.**" block as the single home (it already names the three escape hatches). In ha_device, cut to one clause: "shows hidden entities too, marked `\"hidden\": true` — naming a device means you want its whole instrument panel." In ha_get_state, drop the hidden-but-enabled/salience sentence pair and let the block carry it. Consistent with model-facing-context.md: keep the rule at one point of application, pointers elsewhere.
...
- **[medium/economy]** `talents/documents.md` — documents_discover: `search: "on_request"` bullet (lines 201-204) and funnel-precondition paragraph (lines 216-219) *(~30 tok/activation)*
  - The empty-search-proves-nothing doctrine is stated twice in full within the documents_discover node.
  - Fix: Keep the vivid bullet (it sits at the point of application, beside the field that causes the miss) and compress the later paragraph to one sentence: "So when a search comes back empty, check `doc_roots` and search any unnamed on_request root before concluding the thing does not exist."

- **[low/economy]** `talents/ha.md` — ha root ha_device (lines 100-108) and ha_control ha_call_service targets paragraph (lines 412-415) *(~30 tok/activation)*
  - The HA-2026.8 one-device-per-integration twin explanation is duplicated across the root and ha_control, which co-load in practice since ha_control is reached through the root and tag activation only accumulates.
...
- **[low/economy]** `talents/documents.md` — documents_mutate_content: doc_create (lines 319-322), doc_write (lines 345-346), doc_edit (lines 371-375) *(~20 tok/activation)*
  - The contract-owned-document exception (use contact_dossier_write / generated output tools instead) is restated in all three body-mutation tool sections of one node.
  - Fix: Hoist one statement to the node's opening paragraph ("All three operate on *ordinary* documents; a contract-owned document — a contact dossier, a loop output — writes through its own structured tool: `contact_dossier_write`, `publish_output_*`/`replace_output_*`"), keep doc_edit's worked drift example since it carries the why, and drop the other two restatements. Net savings is modest since the hoisted statement replaces most of doc_create's copy.

- **[low/economy]** `talents/documents.md` — documents root, paragraph after disambiguation table (lines 49-52) *(~40 tok/activation)*
  - The paragraph after the disambiguation table restates two of the table's own rows, including their examples.
  - Fix: Delete the paragraph; the table already carries both facts at the border where they matter.

- **[low/tone]** `talents/documents.md` — root line 13 ("so the model can think in documents"), documents_mutate lines 301-303 ("The model never has to know the filesystem path"); also talents/ha.md line 11 ("the largest lever this agent has")
  - Three third-person voice wobbles ("the model", "this agent") break the past-self-to-present-self second-person voice both files otherwise hold (docs/talent-authoring.md: "the model — past-self writing for present-self").
...
### Communication cluster *(single-pass except high-priority items — see Verification status)*

- **[high/doc-code-drift]** `talents/contacts.md` — contacts_vcf node, '## Generate a QR code' (~lines 471-484)
  - The QR example passes recipient_trust_zone for a non-self contact, but the runtime applies trust-zone filtering only when name is "self", so the example teaches a silently ignored parameter and contradicts the talent's own export section two paragraphs earlier.
  - Fix: Change the QR example to name: "self" (the only case where the parameter does anything) and add one line: 'As with contact_export_vcf, filtering applies only to the self card — a non-self QR export carries full data, and the only size lever is the contact record itself.' Note the tool's own over-capacity error ('Use recipient_trust_zone to reduce fields', tools.go:991) gives the same dead-end advice for non-self contacts and is worth a follow-up code fix.

- **[high/assembly]** `talents/contacts.md` — contacts_vcf node, stray bullet after '## Cross-references' (lines 499-502); root '## Choose by the shape of your question' (lines 41-58)
...
- **[high/anti-pattern]** `talents/notifications.md` — notifications_ask node, thane_now paragraph (~lines 240-244)
  - request_ai_escalation is registered under the notifications tag with a handler that returns not-implemented (the documented phantom-symmetry case), but the talent never names it — instead the thane_now paragraph ends with a contrast clause the model cannot decode.
...
- **[medium/economy]** `talents/contacts.md` — root node, '## Constants across all branches', 'save merges; forget soft-deletes' bullet (~lines 73-83) *(~100 tok/activation)*
  ...
  - Fix: Before: the full 110-word bullet. After: '**save merges; forget removes.** There is no separate update tool — `contact_save` on an existing name IS the update, and `contact_forget` has no undo the tools can reach, so look up before forgetting. Merge mechanics live in `contacts_save`.' The leaf keeps the authoritative detail.
...
- **[medium/economy]** `talents/notifications.md` — notifications_ask node, opening paragraph (~lines 208-224) *(~60 tok/activation)*
  ...
  - Fix: Before: the two quoted clauses. After: '(always available — no activation needed)' and drop the metacog/ego sentence entirely; 'the canonical service-loop → operator attention path' already covers internal loops. Keep the excellent 'send the ask, finish the turn, and let the answer find you' body.

- **[medium/tone]** `talents/contacts.md` — contact_owner section (~lines 209-211); also notifications.md Constants (~line 95), signal.md disambiguation (~line 17), notifications_resolve (~line 339)
  - The three large trees repeatedly slip from the corpus's second-person past-self voice into third-person editor framing ('the model'), worst where one sentence mixes third and first person.
  - Fix: Sweep 'the model' to second person: 'Right tool when you need to assert...', 'the misroute you walk into', 'When you're inside an inbound Signal conversation', 'Your job is to extract...'. Also fix the two changelog-tense leaks: 'A zone now confers inherited authority' (contacts.md:239) → 'A zone confers...', and see comms-mark-changelog for email.md.

- **[medium/economy]** `talents/email.md` — email_organize node, '## Mark messages' paragraph (~lines 296-300) *(~30 tok/activation)*
  ...
  - Fix: Before: the quoted five clauses. After: '`add: true` (the default) adds the flag; `add: false` removes it. Single-message mode accepts `uid` instead of `uids`.' (Default verified at tools.go:175.)

- **[low/economy]** `talents/contacts.md` — root node: paragraph after the disambiguation table (lines 35-39) and cross-reference bullets for archive/memory (lines 108-109, 113-115) *(~70 tok/activation)*
...
- **[low/assembly]** `talents/interactive-doctrine.md` — whole file, alongside talents/interactive-communication.md (both `tags: [interactive]`) *(~25 tok/activation)*
  - Two flat posture files share the [interactive] tag and always co-load, so the split buys no decision-time selectivity and fails the authoring rule 'default to single-talent; split only when the combined file becomes unreadable' — this is not the blessed posture/examples split, it is posture split from posture.
...
### Operations cluster *(single-pass except high-priority items — see Verification status)*

- **[high/economy]** `talents/forge.md` — forge_known_pr node, '## Read the PR' section, lines ~167-220 *(~140 tok/activation)*
  - Five of the six read-tool JSON examples are shape-identical {repo, number} payloads that teach nothing after the first one.
  - Fix: Keep the forge_pr_get example and the forge_pr_diff example (it adds max_lines). Replace the other four JSON blocks with one line: 'forge_pr_files / forge_pr_commits / forge_pr_reviews / forge_pr_checks all take the same {repo, number} shape.' The per-tool prose bullets stay. Also drop the sixth repeat of the same literal in forge_review_loop step 1 — its parenthetical already names the four tools.

- **[high/border]** `talents/forge.md` — forge_review_loop node, steps 1 and 5, lines ~404-406 and 432-441
  - The canonical review-loop workflow routes its thread-read and reply/resolve steps through the gh CLI with no mention that this requires exec (shell tag, off-by-default) and a GitHub-specific authenticated CLI — a model with only forge tags active cannot execute steps 1b or 5.
...
- **[medium/economy]** `talents/session.md` — '## Choosing the right one', lines 165-177 *(~90 tok/activation)*
...
- **[medium/economy]** `talents/memory.md` — '## Two stores share this tag', lines 86-105 *(~90 tok/activation)*
  - The persistent-facts vs session-working-memory distinction is stated four times in the file — disambiguation table row, this dedicated section, the operational 'Session working memory' section, and the cross-references — and this section is the compressible copy.
...
- **[medium/economy]** `talents/archive.md` — root node, paragraph after the disambiguation table, lines 28-40 *(~80 tok/activation)*
  - The paragraph following the disambiguation table re-walks all four surfaces with one example question each, then a closing paragraph asks the same four-way question a third time — three layers where the table plus one contrast would carry the border.
  - Fix: Keep the table and the final 'When you're not sure, ask...' litmus (it is the compressed form). Cut the example-walk paragraph, or fold its sharpest pair into the table rows themselves ('what did I tell the user about X' / 'what do I know about X') — the VLAN examples repeat the same subject four times to make a point one contrast makes.
...
- **[low/economy]** `talents/memory.md` — '## Storing a fact', category constraint block, lines 121-127 *(~45 tok/activation)*
...
- **[low/economy]** `talents/memory.md` — '## Session working memory' closing paragraph (lines 219-223) and final cross-references bullet (lines 236-240) *(~40 tok/activation)*
  - The operational-vs-craft split with working-memory.md is stated twice within seventeen lines, in nearly the same words.
  - Fix: Keep the cross-references bullet (that is the designated home per the leaf grammar) and delete the section-closing paragraph, leaving only its first clause pointing at working-memory.md if a local pointer is wanted.

- **[low/economy]** `talents/archive.md` — archive_text node: 'Read distilled first' tail (lines 110-113) and '## Following the trail' (lines 136-141); archive_session node '## The canonical pairing' (lines 279-294) *(~60 tok/activation)*
...
- **[high/doc-code-drift]** `talents/feeds.md` — ## Follow a feed (lines 29-49)
  ...
  - Fix: Open the section with the name: "`media_follow` registers the feed:" before the JSON. The talent names media_feeds, media_unfollow, and even sibling tools (mqtt_wake_add, forge_repo_follow) but not its own follow door; the leaf-grammar success criterion is a working call adapted from the leaf, which needs the tool name.
...
- **[high/economy]** `talents/feeds.md` — intro lines 8-17 + "notify: false is the third configuration mode" lines 51-57 *(~90 tok/activation)*
  - notify: false is explained twice nearly verbatim, and both passes restate what the notify parameter's own schema description already says on every activation.
...
- **[medium/economy]** `talents/feeds.md` — second JSON example lines 59-70 + ## Pairing with service loops lines 101-114 *(~140 tok/activation)*
  ...
  - Fix: Delete the second JSON block and its lead-in; collapse ## Pairing with service loops into one sentence attached to the first example ("point wake_loop at a thane_loop_create service loop and let its task judge each entry — see loops_examples_curate"), keeping the cross-reference bullet as the only pointer. Also drop the aside "the output document is now optional" — changelog voice, and it is loops doctrine already stated in loops-examples.md ("may maintain a managed markdown document, but need not"). Saves ~140 tokens.

- **[medium/economy]** `talents/attachments.md` — ## Why this is a receive-side surface (lines 117-129) + intro parenthetical (lines 13-14) *(~80 tok/activation)*
  ...
  - Fix: Before: 13 lines of why. After: two sentences — "No tool sends an attachment outward, on any channel. If asked to send a photo, describe it in prose and ask the human to attach it from their own client." Fix the intro parenthetical to "those are channel-side concerns, and no channel tool currently takes one." Saves ~80 tokens and removes the internal tension.

- **[medium/stale]** `talents/files.md` — opening lines 7-10 + ## Trust these instincts (lines 34-40) *(~110 tok/activation)*
  - Everything outside the disambiguation table reads like a pre-split stratum of documents doctrine: the opener has no files referent, and instinct bullet 1 contradicts the file's own table by claiming semantic refs for the files surface.
  - Fix: Before: koan + two posture paragraphs + four instinct bullets. After: one framing line ("`files` is raw filesystem access inside the workspace") + the table + the .md-extension gotcha + keep only the memory-border bullet ("long content lives in files or documents, never `remember_fact`"). The read-before-widening posture already lives in web.md and documents doctrine. Saves ~110 of the file's ~459 tokens.
...
- **[medium/economy]** `talents/companion.md` — intro lines 16-22 + observations bullet lines 33-39 *(~70 tok/activation)*
  ...
  - Fix: Cut the duplicated sentences from the talent and keep only what the block doesn't say: the observed_ago vs received_ago backlog distinction, and that offline is normal, not a fault. While editing, trim the 131-char teaser (over the ~100 guidance) to e.g. "Open when a paired device holds the answer — Mac calendar, contacts, reminders; iPhone observations." Saves ~70 tokens per activation.
...
- **[low/economy]** `talents/attachments.md` — ## Browse filter bullets (lines 41-45) *(~50 tok/activation)*
  ...
  - Fix: Delete the bullet list; keep the two genuinely additive gotchas that follow — the `signal-15551234567` hyphen/bare-digits conversation_id shape (verified against sanitizePhone in signal/bridge.go:1343) and the no-time-range-filter warning. Saves ~50 tokens.

- **[low/economy]** `talents/web.md` — lines 7-10 and bullet 1 (line 16) *(~35 tok/activation)*
...
### Anthropic caching (code + policy doc) *(single-pass except high-priority items — see Verification status)*
...
- **[high/stale]** `internal/model/fleet/providers/anthropic.go` — minCacheablePrefixTokens, lines 843-860 (pinned by anthropic_test.go:371-391) *(~3000 tok/activation)*
  - The minimum-prefix guard uses 4096 tokens for all Opus models, but the live Anthropic doc says Claude Opus 4.8 — the Opus model in the production fleet (config.go:3026) — needs only 1024, so the guard strips valid breakpoints and silently disables caching on compact (task/delegate-mode) prompts.
  - Fix: Replace the flat per-family switch with a per-version table matching the live doc (opus-4-8→1024, opus-4-7→2048, opus-4-6/4-5→4096), update the pinned test values, and update the docs/prompt-caching.md table (line 137 says "Claude Opus 4.x | 4096") in the same change.
...
- **[high/caching]** `internal/model/fleet/providers/anthropic.go` — applyCacheBreakpointGuards step 1, lines 916-940 *(~5000 tok/activation)*
  - The under-minimum guard measures the prefix using system-block characters only, but Anthropic's cacheable prefix starts at tools (hierarchy tools -> system -> messages), so with Thane's tens-of-kilobytes tool schemas most under_minimum_prefix drops are false positives that discard breakpoints whose true prefix is far above the minimum.
  - Fix: Seed prefixChars with the serialized tool-definition length (the same JSON marshalling estimateToolDefsContextTokens uses) before walking system blocks, so the estimate matches what the API actually measures. This is the cheap fix that also makes findings 1-2 far less likely to bite in practice.
...
- **[medium/border]** `internal/model/fleet/providers/anthropic.go` — minCacheablePrefixTokens default arm (lines 857-859) and anthropicMaxTokens (lines 819-831)
  - Claude 5-generation model families now published by Anthropic (Opus 5/Fable 5/Mythos 5 minimum 512, Sonnet 5 minimum 1024, Mythos Preview 2048) have no guard coverage — Fable/Mythos names match no substring and fall to the strictest 4096 default (8x too strict), and anthropicMaxTokens caps them at the legacy 4096 output ceiling.
  - Fix: Add explicit cases for the 5-generation names (fable, mythos, opus-5, sonnet-5) before the fleet adopts them, and extend the repo doc's minimum-prefix table beyond 4.x. Today's config fleet is all 4.x, so this is preparatory rather than an active loss.
...
- **[medium/caching]** `internal/runtime/agent/loop.go` — promptSectionCacheTTL TALENTS TAGGED 5m (lines 1379-1380); per-iteration ToolDefs (lines 2237-2243); currentTools FilterByTags (lines 2141-2153)
...
- **[low/doc-code-drift]** `docs/prompt-caching.md` — stability table lines 33-50 vs loop.go:1210,1220
  ...
  - Fix: Rename ACTIVE CAPABILITIES to ACTIVE TAGS in the table and add a SESSION ORIGIN CONTEXT row (volatile — origin/contact JSON with per-run applied rules). The section name is stated to be "the stable contract for caching policy", so the table should match the emitter exactly.
...
- **[low/border]** `internal/model/fleet/providers/anthropic.go` — applyCacheBreakpointGuards (912-1004) and promptSectionCacheTTL (loop.go:1375-1384)
  - The live doc's mixed-TTL ordering constraint — 1h cache entries must appear before any 5m entries — is satisfied today only by construction (tool 1h, then 1h system run, then 5m tail) and is neither documented in docs/prompt-caching.md nor validated by the guard, so a future TTL-map edit placing a 1h section after TALENTS TAGGED would produce an invalid request with no local diagnosis.
  - Fix: Add a cheap ordering check to applyCacheBreakpointGuards (demote any 1h marker that follows a 5m marker to 5m, with a WARN and a CacheBreakpointDrop-style record) and add the constraint to the doc's Anthropic Anti-Patterns list.


### Context assembly (runtime providers) *(single-pass except high-priority items — see Verification status)*
...
- **[high/caching]** `internal/runtime/agent/core_context.go` — readEgoFromProvenance, lines 264-266 (with loop.go:1377 TTL mapping)
  ...
  - Fix: FormatDeltaOnly emits second-granular deltas under 1h and minute-granular under 48h, so the EGO bytes change on essentially every turn for two days after any ego edit, invalidating the prefix cache mid-run (EGO + INJECTED CONTEXT + RUNTIME CONTRACT + TOOL CALLING CONTRACT + TALENTS ALWAYS ON get re-written as cache_creation tokens each turn — easily 5-15k tokens at 1.25x pricing instead of 0.1x reads). Either quantize the freshness line to a coarse bucket that only changes daily (FormatDayDelta-style day words), move the ego-freshness fact into CURRENT CONDITIONS/live state, or drop the delta and keep only the revision count, which changes exactly when the content changes.

- **[high/border]** `internal/runtime/agent/channel_provider.go` — channelDefaults map, lines 117-128; rendered at line 232 into the Continuity bucket *(~160 tok/activation)*
  - ~650 bytes of static behavioral guidance for the Signal channel is injected as a JSON string field inside the volatile CONTINUITY CONTEXT bucket on every Signal turn, violating the instructions-vs-data separation rule and paying full uncached token price for prose that never changes.
  - Fix: docs/model-facing-context.md: "Behavioral guidance belongs in talents and prompts. Runtime facts belong in context providers... Do not hide instructions inside what claims to be data." Channels already pin capability tags (capabilityScope.PinChannelTags), so move the Signal style guidance to a signal-tagged talent — it then rides TALENTS TAGGED with a 5m cache TTL instead of the uncacheable volatile bucket — and keep the envelope to runtime facts: source, contact profile, trust policy. Before: {"source":"signal","note":"Signal (mobile chat app). Plain text only...","contact":{...}}. After: {"source":"signal","contact":{...}} plus a signal.md talent carrying the prose.

- **[high/other]** `internal/runtime/agent/loop.go` — line 2107 (usageInfo.TokenCount) vs context_sizing.go:62-79
  - The CONTEXT USAGE percentage the model reads is computed from messages alone while tool schemas — which the same file's comments call "tens of kilobytes of JSON schema, large enough to overrun a window sized from the messages alone" — are excluded, so the model acts on a systematically understated occupancy figure it has no way to correct.
  - Fix: Use estimateRequestContextTokens (messages + tool defs) for the reported TokenCount, or report both numbers explicitly. On a 200K window the gap is a few percent, but on small local-runner windows the tool surface can be 20-40% of the window — a model deciding whether to compact or defer a large pull is doing so on the wrong denominator. This is precisely the "do the math before the model sees the data" rule failing at the last step. While in the block: req:<id> in the IDs line is per-turn operator forensics the model never acts on; consider dropping it from the prompt (it already rides request-detail retention).
...
- **[medium/anti-pattern]** `internal/runtime/agent/context_discriminator.go` — materializeContextAdvertisements, lines 218, 232-276, 298-308
  ...
  - Fix: The Context Advertisements contract says "a capped rail must never read as a complete one" — an offer that was selected and then dropped for overrunning its own estimate was a genuinely selectable offer the budget policy refused, and it vanishes from the model's view entirely. Increment a materialization-drop counter in each continue path and fold it into the rendered line. Derive the pull door from the withheld advertisements' Source/Kind fields instead of hardcoding doc_search, which is the wrong door for e.g. a forge or notifications advertiser.
...
- **[medium/assembly]** `internal/runtime/agent/prompt_order_test.go` — assertPromptSectionOrder call, lines 70-84; loop.go:2108-2113
  ...
  - Fix: Extend the ordering test through the full documented table (the caching doc's ordering invariant is only as strong as its regression guard), route CONTEXT USAGE through the same appendTracked path so its content starts at content and its position is pinned last, and add a case that exercises a non-empty ToolCallingContract profile so the 1h-cached TOOL CALLING CONTRACT's position ahead of TALENTS TAGGED is guarded rather than assumed.
...
- **[medium/economy]** `internal/runtime/loop/loop_self_context.go` — SelfContextMarkdown, line 95 *(~50 tok/activation)*
  ...
  - Fix: Switch to the compact marshal helper. The ~25-field payload ships every iteration in the LIVE STATE bucket; two-space indentation plus per-line newlines adds roughly 150-250 bytes per render for zero model value (the doc's own convention: "compact JSON objects for single records with stable fields"). Before: multi-line indented block; after: one compact object, same schema.
...
- **[low/tone]** `internal/runtime/agent/core_context.go` — readEgoFromProvenance, line 265
  ...
  - Fix: Rephrase so the field's nature is explicit from the model's vantage point, e.g. "(updated -26h45m, revision 3; last change: \"third-write\")", or drop the message entirely — the revision count already conveys churn. Coordinate with ego-cache-churn-1: whatever survives should not include a fine-grained delta in this cached section.

- **[low/doc-code-drift]** `internal/runtime/agent/channel_provider.go` — InteractionRef doc comment, lines 87-89; also internal/state/awareness/context_usage.go:16-21
  ...
  - Fix: Fix both comments to describe the actual emitted shape. These comments are the spec future providers copy from — the channel_provider tests already hardcode the obsolete "-3600s"/"-7200s" shapes, evidence the stale doc is propagating. For context_usage.go, re-document Model as the router-selected model and Routed as "a router made this selection".

- **[low/border]** `internal/runtime/agent/channel_overview.go` — channelEntry, lines 97-107
...
- **[low/doc-code-drift]** `internal/state/awareness/conditions.go` — package doc comment lines 5-9 and H1 heading at line 83
  - The package comment claims Current Conditions is "placed early in the system prompt (after persona and inject files, before talents)" with "heading level (H1) signals importance", but the section is actually rendered last (loop.go:1262, per the stable-prefix-first cache policy), leaving a stale rationale and the prompt's only H1 heading among uniformly H2 sections.
  - Fix: Rewrite the comment to reflect the volatile-last placement and its caching rationale, and demote the heading to "## Current Conditions" to match every other section (appendTrackedMarkdown level 2, "## Active Tags", "## Behavioral Guidance") — a lone H1 at the prompt tail is a leftover emphasis signal from the abandoned early-placement design.
