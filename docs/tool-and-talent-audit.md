...
| **Wrong-data-store** | Talent points at a real tool that mutates a different store than the reader expects | `add_entity_subscription` (conversation-wide) vs the since-retired `update_entity_subscriptions` (loop-scoped) — caught in PR-A; the split itself was dissolved by #1209's owner parameter |
...
| **Phantom symmetry** | Tool / type / API surface exists because completing a symmetric pattern was satisfying, not because the case was demonstrated. Registers in the catalog, sits in the talent corpus, the model tries to use it, the model hits a stub. Distinct from "ghost tool reference" (where the talent points at a tool that doesn't exist); phantom symmetry is the *opposite* — the tool exists in the catalog but the implementation never landed. | `request_ai_escalation` registered as the sync-AI companion to `request_human_escalation`. Handler returns "not yet implemented" on every call; the working AI-escalation paths (calling `thane_now` from a premium-routed turn so the delegate inherits the higher-capability routing; orchestrator-side routing-profile bump) make the symmetric slot unnecessary. The audit catches this by tracing the handler, not just reading the description. |
